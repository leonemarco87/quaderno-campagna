...
export function Magazzino() {
  ...
  const statoScadenza = (row) => {
    ...
    const days = differenceInDays(new Date(row.scadenza), new Date())
    if(days < 0)   return <span className="badge-red">🔴 Scaduto</span>
    if(days <= 30) return <span className="badge-yellow">🟡 In scadenza</span>
    return <span className="badge-green">🟢 OK</span>
  }
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="label">Tipo concime</label>
              <select className="input" value={form.tipo_concime} onChange={e=>set('tipo_concime',e.target.value)}>
                <option value="">— Seleziona —</option>{TIPI_CONC.map(t=><option key={t}>{t}</option>)}
              </select></div>
            ...
            <div><label className="label">Metodo distribuzione</label>
              <select className="input" value={form.metodo_distribuzione} onChange={e=>set('metodo_distribuzione',e.target.value)}>
                <option value="">— Seleziona —</option>{METODI.map(m=><option key={m}>{m}</option>)}
              </select></div>
    ...
    <div className="fade-in space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="card bg-limone-100 border-0 text-center">
          ...
          <p className="text-2xl font-bold font-mono text-limone-600">{totKg.toLocaleString('it-IT')} kg</p>
        </div>
        <div className="card bg-verde-50 border-0 text-center">
...
const TIPI_SCA=['Trattamento fitosanitario','Irrigazione programmata','Concimazione',
  'Manutenzione macchine','Potatura','Raccolta','Pagamento fornitore','Scadenza documento','Rinnovo patentino','Analisi terreno','Altro']
...
export function Scadenzario() {
  ...
  const cols=[
    ...
    { key:'stato',        label:'Stato', width:100, render:(_,r)=>daysBadge(r) },
